...
export function updateResultTable(products) {
    ...
    let macroInfo = '';
    if (products.length > 0 && products[0].hasOwnProperty('protein') &&
        products[0].hasOwnProperty('fat') && products[0].hasOwnProperty('carbohydrate')) {
    ...
    const resultTemplate = html`
        <div class="price-card">
            <h3>Total price per day: <span class="price-highlight">${total_price} EUR</span></h3>
        </div>
        ${macroInfo}
    ...
    // Initialize the pie chart if the macro distribution element exists
    if (document.getElementById('pie')) {
        // We need to import and call makePie dynamically, as it's a module
        import('./pie.js').then(module => {
            module.makePie(document.getElementById('pie'));
        });
    }
}
