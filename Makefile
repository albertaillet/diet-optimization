...
CIQUAL_XML_ZIP := $(DATA_DIR)/XML_2020_07_07.zip
...
nutrient-map-reformat: $(NUTRIENT_MAP_RE)
	DATA_DIR=$(DATA_DIR) uv run --no-project scripts/nutrient_map/nutrient_map_reformat.py

nutrient-map-update-counts:
	DATA_DIR=$(DATA_DIR) uv run scripts/nutrient_map/nutrient_map_update_counts.py

nutrient-map-update-ciqual:
	DATA_DIR=$(DATA_DIR) uv run --no-project scripts/nutrient_map/nutrient_map_update_ciqual.py
...
CIQUAL_DIR := $(DATA_DIR)/ciqual2020
XML_TO_CSV := uv run --no-project --with lxml scripts/xml_to_csv.py
unzip_and_process_ciqual: $(CIQUAL_XML_ZIP)
	[ -d $(CIQUAL_DIR) ] && rm -r $(CIQUAL_DIR) || true
	unzip -o $(CIQUAL_XML_ZIP) -d $(CIQUAL_DIR)
	$(XML_TO_CSV) $(CIQUAL_DIR)/alim_2020_07_07.xml $(CIQUAL_DIR)/alim.csv
	$(XML_TO_CSV) $(CIQUAL_DIR)/alim_grp_2020_07_07.xml $(CIQUAL_DIR)/alim_grp.csv
	$(XML_TO_CSV) $(CIQUAL_DIR)/compo_2020_07_07.xml $(CIQUAL_DIR)/compo.csv
	$(XML_TO_CSV) $(CIQUAL_DIR)/sources_2020_07_07.xml $(CIQUAL_DIR)/sources.csv
	$(XML_TO_CSV) $(CIQUAL_DIR)/const_2020_07_07.xml $(CIQUAL_DIR)/const.csv
...
$(NNR_EXTRACTED_TABLES): $(NNR_HTML)
	DATA_DIR=$(DATA_DIR) uv run --no-project --with bs4 scripts/recommendations_nnr2023/recommendations_extract_tables.py

$(NNR_SUMMARY_CSV): $(NNR_EXTRACTED_TABLES)
	DATA_DIR=$(DATA_DIR) uv run --no-project scripts/recommendations_nnr2023/recommendations_summarize_general.py
...
opt:
	DATA_DIR=$(DATA_DIR) uv run dietdashboard/app.py
...
template:
	DATA_DIR=$(DATA_DIR) uv run --no-project scripts/template_nutriments_query.py
